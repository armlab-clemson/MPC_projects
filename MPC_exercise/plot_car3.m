function plot_car3(xx,xx1,xs,N,L,t,u_cl)
x_r_1 = [];
y_r_1 = [];
...
%% Parking Line
 ...
 y_p3= 18*ones(1,100);
 y_p4= 14*ones(1,100);
 x_p= linspace (18.8,21.2);
 y_p5= linspace(14.75,25.25);
 x_p2= 21.2*ones(1,100);
 
 %% Obstacles
 w= 1.5; radius= 1.5071;
 x_obs= 20; y_obs1= 23.5; y_obs2= 16.5; x_obs3= 12; y_obs3=17;
 ang=0:0.005:2*pi;
 ...
 x_obs3_rect = [x_obs3 - L/2, x_obs3 + L/2,x_obs3 + L/2,x_obs3 - L/2];
 y_obs3_rect = [y_obs3 + w/2, y_obs3 + w/2,y_obs3 - w/2,y_obs3 - w/2];
 %% Safe radius
 x_circ=radius*cos(ang);
 y_circ=radius*sin(ang);
%% Reference Point and Pose
 ...
 for k=1:size(xx,2)
    ...
    hold on
    fill(x_obs3_rect,y_obs3_rect,'r','HandleVisibility','off');
    hold on
    fill(xref_rect,yref_rect,'k','HandleVisibility','off');
    hold on
    plot(x_obs+x_circ, y_obs1+y_circ,'--r','LineWidth',3);
    hold on
    plot(x_obs+x_circ, y_obs2+y_circ,'--r','LineWidth',3);
    hold on
    plot(x_obs3+x_circ,y_obs3+y_circ,'--r','LineWidth',3);
    hold on
    plot(x2+x_circ, y2+y_circ,'--r','LineWidth',3);
    hold on
